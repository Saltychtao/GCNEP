...
def build_graph_from_adj_matrix(adj_matrix,device,norm_type):
    ...
    for i in range(len(adj_matrix)):
        for j in range(len(adj_matrix)):
            ...
            if num_of_edges == 0:
                ...
                if num_of_edges <= 0:
                    continue
                n = min(1+round(math.log(num_of_edges,10)),10)
            for k in range(n):
                src.append(i)
                dst.append(j)
    # g.from_networkx(G)
    ...
    norm = comp_deg_norm(g,norm_type)
...
def comp_deg_norm(g,norm_type):
    in_deg = g.in_degrees(range(g.number_of_nodes())).float().numpy()
    if norm_type == 'gcn':
        in_deg = np.sqrt(in_deg)
    norm = 1.0 / in_deg
